...
class TTTFunction(autograd.Function):
    @staticmethod
    def forward(ctx, Q, K, V, eta=0.9, block_size=64):
        B, L, D = Q.shape
...
# 融合算子封装为可直接调用的模型类
class FusedTTT(nn.Module):
    def __init__(self, d_model: int, eta=0.9, block_size: int = 64):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        K = self.Wk(x)
        V = self.Wv(x)
        return TTTFunction.apply(Q, K, V, self.eta, self.block_size)
